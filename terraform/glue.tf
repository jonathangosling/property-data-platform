...
resource "aws_glue_job" "silver" {
  ...
  default_arguments = {
    "--landing_path"              = local.landing_path
    "--iceberg_path"              = local.iceberg_path
    "--catalog_database"          = aws_glue_catalog_database.property_data.name
    "--job-bookmark-option"       = "job-bookmark-enable"
    "--enable-glue-datacatalog"   = "true"
    "--datalake-formats"          = "iceberg"
    "--conf"                      = "spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions --conf spark.sql.catalog.glue_catalog=org.apache.iceberg.spark.SparkCatalog --conf spark.sql.catalog.glue_catalog.warehouse=${local.iceberg_path} --conf spark.sql.catalog.glue_catalog.catalog-impl=org.apache.iceberg.aws.glue.GlueCatalog --conf spark.sql.catalog.glue_catalog.io-impl=org.apache.iceberg.aws.s3.S3FileIO"
  }
